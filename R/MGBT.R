...
function(x, alphaout=0.005, alphazeroin=0.10, n2=floor(length(x)/2),
   ...
   # WHA note that by TAC using 1e-8 as "small" he has assumed that the MGBT will
...
# In MGBTcohn2013, TAC uses recursion back to that same function. The MGBTnb() recurses
# to MGBTcohn2013 and not back to itself, which appears the only real difference between the two.
"MGBTnb" <-
function(x, alphaout=0.005, alphazeroin=0.10, n2=floor(length(x)/2),
            napv.zero=TRUE, offset=0) {
   if(length(unique(x)) == 1) {                           # WHA error trapping
...
# This is a slower function in general practice between some omegas (w) get doubly computed.
...
function(x, alphaout=0.005, alphain=0, alphazeroin=0.10, n2=floor(length(x)/2),
   ...
   for(i in seq(1,n2,by=+1)) { # NOTE LITERALLY SWEEP IN as per 17C
      ...
      if((pv <= alphazeroin ) & (j2==i-1)) { j2 <- i; next } # effect of sweep in, NOTE LESS THAN EQUAL
      break
   }
   k <- max(j1,j2) # as per B17C
   if(k >= n) return(list(index=idx, klow=0, LOThresh=0))
   idx <- c(n,n2,j1,NA,j2); names(idx) <- txt
   return(list(index=idx, klow=k,
               LOThresh=ifelse(k > 0, x[k+1]+offset, 0)))
}


# This code base follows more along lines of WHA idioms.
...
function(x, alphaout=0.005, alphain=0, alphazeroin=0.10, n2=floor(length(x)/2),
   ...
   return(list(index=idx, omegas=w[1:n2], x=x[1:n2], pvalues=pv, klow=k,
               LOThresh=ifelse(k > 0, x[k+1]+offset, 0)))
}
...
function(pvs, alphaout=0.005, alphain=0, alphazeroin=0.1) {
   ...
   if(j1 < n2) { # Late addition station 08020500 causes j1 to be n2
